...
const rule: Rule.RuleModule = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Disallow Reactstrap components and suggest modern alternatives',
      category: 'Best Practices',
      recommended: true,
      url: 'https://github.com/vahidmohammadi/you-dont-need-bootstrap#reactstrap',
    },
    messages: { noReactstrapComponents: "Avoid Reactstrap component '{{componentName}}'" },
    schema: [],
  },
  ...
  create(context): Rule.RuleListener {
    ...
    return {
      ...
      ImportDeclaration(node: Node) {
        ...
        if (node.source.type === 'Literal' && node.source.value === 'reactstrap') {
          // Handle named imports: import { Button, Alert } from 'reactstrap'
          node.specifiers?.forEach(specifier => {
            if (specifier.type === 'ImportSpecifier') {
              const localName = specifier.local.name;
              const importedName =
                specifier.imported.type === 'Identifier' ? specifier.imported.name : localName;
      ...
      JSXOpeningElement(node: Node) {
        ...
        // Handle namespace component: <Reactstrap.Button />
          ...
          if (
            ...
            const namespace = node.name.object.name;
            if (namespace === namespaceImport) {
              componentName = `${namespace}.${node.name.property.name}`;
              originalName = node.name.property.name;
            }
          }
        }
